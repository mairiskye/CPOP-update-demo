...
#resource id obtained by visiting dataset on webpage and coping id from browser url
...
healthy_BMI$ClinHealthyWeight <- 100 * healthy_BMI$ClinHealthyWeight
...
p1_bmi_data <- left_join(healthy_BMI, code_lookup_table, by = "CA") %>%
  arrange(CPP) %>%
  rename("Year" = SchoolYear, "value" = ClinHealthyWeight) %>%
